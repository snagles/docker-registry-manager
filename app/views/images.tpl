          ...
          <div class="col-md-6">
            <div class="row col-md-10">
              ...
              <ul class="well well-md">
                <li><strong>Layers:</strong> {{len .tag.DeserializedManifest.Layers}}</li>
                <li><strong>Size:</strong> {{bytefmt .tag.Size}}</li>
                <li><strong>OS:</strong> {{.tag.Os}}</li>
                <li><strong>Architecture:</strong> {{.tag.Architecture}}</li>
                <li><strong>Docker Version:</strong> {{.tag.DockerVersion}}</li>
                <li><strong>Created:</strong> {{timeAgo .tag.Created}}</li>
                <li><strong>Last Updated:</strong> {{timeAgo .tag.LastModified}}</li>
              </ul>
            </div>
            <div class="row col-md-10">
              <h4>Dockerhub <a href="{{.dockerHub.ImageURL}}"><i style="vertical-align:bottom; color:black;"class="fa fa-external-link" aria-hidden="true"></i></a></h4>
              <div class="well well-md">
                ...
                {{else}}
                  ...
                  {{ if eq $length 0 }}
                    ...
                    <div><span>Both local and remote images have {{len .tag.DeserializedManifest.Layers}} layers and are {{bytefmt .tag.Size}}.</span></div>
                  {{else}}
                    <div><strong>Image is not up to date.</strong></div>
                    <div><span>There are {{len .dockerHub.DiffLayers}} different layers out of {{len .tag.DeserializedManifest.Layers}} total layers.</span></div>
                    {{if ne .tag.Size .dockerHub.Size}}
                      {{if gt .tag.Size .dockerHub.Size}}
                      <div><span>The Dockerhub image is {{bytefmtdiff .dockerHub.Size .tag.Size}} smaller</span></div>
                      {{else if lt .tag.Size .dockerHub.Size}}
                      <div><span>The Dockerhub image is {{bytefmtdiff .dockerHub.Size .tag.Size}} larger</span></div>
                      {{else}}
                      <div><span>The images are the same total size {{bytefmt .tag.Size}}</span></div>
                      {{end}}
                    {{end}}
                  {{end}}
                {{end}}
        ...
        <div role="tabpanel" class="tab-pane table-responsive" id="build">
          <table id="datatable" class="table" cellspacing="0" width="100%">
            ...
            <tbody>
              ...
              <tr data-tag-name="">
                <td>{{oneIndex $i}}</td>
                {{if not $history.EmptyLayer}} <td data-toggle="tooltip" data-placement="top" title="{{$history.ManifestLayer.Digest.String}}">{{shortenDigest $history.ManifestLayer.Digest.String}}</td>
                ...
                {{else}} <td data-order="0">0B</td> {{end}}
